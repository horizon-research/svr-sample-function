...
float toRadian(degree a){
    return a.to_float() * PI / 180.0f; // HLS would think 180.0 as double. 180.0f tells it that it's a float.
}
...
void convert(int width,int height,float hp,float ht,int fw,int fh, int fovX,int fovY,int option,float fov[1024][1024][2]) {
    ...
    degree i = 45, j = -30.0; // i = fovY / 2, j = -fovX/2

    degree iincre = 0.08789; // fovY / fh
    degree jincre = 0.05859; // fovX / fw
